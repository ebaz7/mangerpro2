...
async function run() {
  try {
    console.log("Running original query (with TOP 50)...");
    const res = await axios.post('http://80.210.31.176:5000/api/external/v1/query', { query: sqlLastYearOriginal }, { headers: { 'Authorization': 'Bearer s_gate_live_vgr182bwtpoa' } });
    console.log("Success! Row count:", res.data.data.length);
    console.log("Sample rows:", res.data.data.slice(0, 2));
  } catch(e) {
