...
async function run() {
    ...
    const sql = `
        SELECT 
            SUM(CAST(t11.Field_007 AS FLOAT)) as SumAmount,
            SUM(CAST(t11.Field_009 AS FLOAT)) as SumNetAmount,
            SUM(CAST(t11.Field_012 AS FLOAT)) as SumFinalAmount,
    ...
    console.log("DB Sums (op 12):", res.data.data[0]);
}
run();
