...
async function run() {
  try {
    console.log("Running fast filtered query (01% and 04%)...");
    const start = Date.now();
    const res = await axios.post('http://80.210.31.176:5000/api/external/v1/query', { query: sqlFilteredFast }, { headers: { 'Authorization': 'Bearer s_gate_live_vgr182bwtpoa' } });
